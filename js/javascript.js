	...
	$(document).ready(function(){
		$("#sign_up").on("click", function(){
			$.ajax({
				...
				success: function(data){
					$('#upload_avatar, #upload_avatar, .drop_down_sort, #error, #notices, #noticeboards, #notice_upload, .add_content, #loading, .main').fadeOut();
					$('#about_us, #contact_us_form, #upload_avatar, #new_noticeboard, #avatar_upload').addClass('hidden');
...
$(document).on("click", "#change_avatar", function(){
	$('#error, #notices, #noticeboards, #notice_upload, .drop_down_sort, .add_content, #loading').hide();
	$('#about_us, #contact_us_form,  #new_noticeboard').addClass('hidden');
	$("#upload_avatar, #avatar_upload").removeClass("hidden").fadeIn(1500);
})
...
$(document).on("click", "#contact_us", function(){
	$.ajax({
		...
		success: function(data){
			$(' #upload_avatar, #notices, #noticeboards, #notice_upload, #error, .add_content, #loading, #user_reviews, .carousel').hide();
			$('#about_us, .drop_down_sort, #upload_avatar, #new_noticeboard, #avatar_upload').addClass('hidden');
			$('#contact_us_form').fadeIn(1500).removeClass('hidden');
		}
	});
})
...
$(document).on('click', '#about_us_menu', function(){
	$('.add_content, #notices, #noticeboards, #loading, #notice_upload, #user_reviews, .carousel').hide();
	$('#contact_us_form, .drop_down_sort, #upload_avatar, #new_noticeboard, #avatar_upload, #error').addClass('hidden');
	$('#about_us').fadeIn(2000).removeClass('hidden');
...
$(document).on("click", "#text_notice", function(){
	$('#upload_avatar, .drop_down_sort, #error, #notices, #noticeboards, #loading').effect('puff');
	$('#about_us, #upload_avatar, #avatar_upload,  #new_noticeboard, #contact_us_form').addClass('hidden');
...
$(document).on("click", "#audio_notice", function(){
	...
	$('#upload_avatar, .drop_down_sort, #error, #notices, #noticeboards, #loading').hide();
	$('#about_us, #upload_avatar, #avatar_upload,  #new_noticeboard, #contact_us_form').addClass('hidden');
...
function load_notice_uploadform(data){
	...
	$.ajax({
		...
		success: function(data){
			$("#notice_upload").html(data).fadeIn(1500);
			if(type=='text_notice'){
				$('#additional').empty();
...
function upload_text_notice(){
	...
	var noticeboard_name= $("#noticeboard_name").val();
	var title=$("#notice_title").val();
	var desc=$("#notice_description").val();
	var expiry=$("#expiry_date").val();

	if((type=='')||(noticeboard_name=='')||(title=='')||(desc=='')||(expiry=='')){
...
$(document).on("click", "#create_noticeboard", function(){
	$.ajax({
		...
		success: function(data){
			$('.modal').detach();
			$('#upload_avatar, #upload_avatar, .drop_down_sort, #error, #notices, #loading, #noticeboards, #notice_upload').hide();
			$('#about_us, #upload_avatar, #avatar_upload,  #new_noticeboard, #contact_us_form').addClass('hidden');
			$(".add_content").html(data).fadeIn(1500);
		}
	});
});
...
$(document).ready(function(){
	...
	$('#loading').empty();
	$('#loading').html("<img id='loading-anim' src='images/loader.gif'>");	

	setTimeout(function(){load_text_notices()}, 300);
	...
	function load_text_notices(){
		...
		$.ajax({
			...
			url: user_setup,
			data: {"Type": "setup", "User_no": user_no },
			//beforeSend: function(){$('#loading').html("<img id='loading-anim' src='images/loader.gif'>");},
			success: function(data){
				var response=jQuery.parseJSON(data);				
				var no_of_noticeboards= response.noticeboards.count;
				$('#loading-anim').addClass('hidden');
						...
						while(count1<no_of_noticeboards){	//filter the noticeboards into different types(public, request, private)
							...
							while(count2<notice_count){     //filter the notices into different types(public, request, private)
								...
								if((notice_type=='1')&&(notice_ext==(('jpg')||('gif')||('png')))){ 										
									...
									html+="<div><a class='pull-left' href='#'><img style='width: 240px' id='image_for_notice' class='img-thumbnail' src='phplib/noticeimage/"+count2+".jpg'></a></div>";
...
*************************************************/	
	...
	$(document).on('click', '#menu_notice_boards', function(){
		var user_no=$("#user_no").html();
		$('#new_noticeboard').html("<p style='margin-left: 40px'><a href='#' id='create_noticeboard'>Create Noticeboard</a></p>").removeClass('hidden');
		...
		$.ajax({
			...
			success: function(data){
				...
				while(count1<number_of_noticeboards){
					...
					html+="<div><a class='pull-left' href='#'><img width='100' height='50' class='media-object' src='images/noticeboard_icon.png'></div>";
				...
				$('.add_content, #notices, #upload_avatar, #upload_avatar, .drop_down_sort, #error, #noticeboards, #loading, #notice_upload').empty();
				$('#about_us, #contact_us_form, #upload_avatar, #avatar_upload, #error').addClass('hidden');
				$.each(noticeboard_data, function(key, data){
					$('#noticeboards').html(data).fadeIn(1500);
				});
			}
		});
...
$(document).ready(function(){	
	...
	function search(data){
		...
		var term=data;
		var name=term.term;
		var noticeboard_suggestions={
			name: []
		}
		$.ajax({
			type: "POST",
			url: create_noticeboard,
			data: {"Type": "search_noticeboard", "User_no": user_no , "Keyword": name , "Lower": "0" , "Limit": "40"},
			success: function(data){
				var response=jQuery.parseJSON(data);
				var count=response.results.count;
				var i=0;
				while(i<count){
					var noticeboard_name=response.results[i].title;
					noticeboard_suggestions.name.push(noticeboard_name);
					i++;
				}
		...
		console.log(noticeboard_suggestions.name);
		return noticeboard_suggestions;
